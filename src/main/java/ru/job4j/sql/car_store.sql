  ...
  select body.name car_body
  from car_body body
  left join car on (body.id = car.car_body_id)
  where car.name is null;
 
  select eng.name engine
  from engine eng
  left join car on (eng.id = car.engine_id)
  where car.name is null;

  select gear.name gearbox
  from gearbox gear
  left join car on (gear.id = car.gearbox_id)
  where car.name is null;